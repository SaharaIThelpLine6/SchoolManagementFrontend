  ...
  return (
    ...
    >
      {label && (
        <label
          htmlFor={registerKey}
          className={`font-medium text-sm ${labelPosition === 'left' ? 'w-2/5 mb-0 text-end' : 'mb-1.5 block'
            }`}
        >
          <div
          ...
          >
            ...
            {codeSetting && (
              <span
                className="text-green-600 hover:text-green-700 underline text-xs font-semibold cursor-pointer transition-colors"
        ...
        <input
          ...
          className={`w-full font-default rounded-lg border text-sm h-11 px-3 outline-none transition-all duration-200 ease-in-out bg-white text-gray-900
            ${hasError
              ...
              : 'border-gray-300 placeholder:text-gray-400 focus:border-green-500 focus:ring-2 focus:ring-green-200 hover:border-gray-400'
            }
...
//         <label
//           htmlFor={registerKey}
//           className={`text-black font-default ${labelPosition === 'left' ? 'w-2/5 mb-0 text-end' : 'mb-1 block'
//             }`}
//         >
//           <div
//             className={`flex items-center gap-2 ${labelPosition === 'left' ? 'justify-end' : 'justify-between'
...
//           {...register(registerKey, {
//             required: require ? 'এই ফিল্ডটি প্রয়োজনীয়' : false, ...(type === 'number' && {
//               validate: (value) =>
//                 isNaN(Number(value)) ? 'দয়া করে একটি বৈধ সংখ্যা লিখুন' : true,
//             }),
//             ...(type === 'phone' && {
