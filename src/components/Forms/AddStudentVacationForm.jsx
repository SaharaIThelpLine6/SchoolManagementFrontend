...
const AddStudentVacationForm = ({ pageTitle }) => {
  ...
  // Mutation
  const [postStudentsVacation, { isLoading, isSuccess, isError, error }] =
    usePostStudentsVacationMutation();
  ...
  // Reset form when admittedStudent changes
  useEffect(() => {
    if (admittedStudent) {
      reset({
  ...
  }, [status, academicSession, setValue]);
  ...
  if (isVacationTypeLoading || status === "loading") {
    return <LoadingComponent />;
  }

  if (studentsVacationTypeError) {
    Swal.fire({
      icon: "error",
      title: "ছুটির ধরনের তালিকা লোড করতে ব্যর্থ",
...
export default AddStudentVacationForm;
