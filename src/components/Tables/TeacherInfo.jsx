...
import { useGetDesignationQuery, useGetFilteredTeachersQuery } from '../../features/teachers/teachersSlice';
...
const TeacherInfo = () => {
  ...
  // ✅ filterTypeId পরিবর্তন হলে DNID / FilterValue reset হবে (unless 4)
  useEffect(() => {
    if (filterTypeId != 4) {
      setValue("DNID", "");
    }
  }, [filterTypeId, setValue]);

  // ✅ Query Params — fully dynamic
  const queryParams = {
    page: currentPage,
    limit: itemPerPage,
  };

  // Dynamic text search (UserCode / UserName / Mobile)
  if (filterTypeId && filterTypeId != 4 && debouncedFilterValue) {
    queryParams.filterTypeId = filterTypeId;
    queryParams.filterValue = debouncedFilterValue;
  }

  // ✅ DNID filter (only when filterTypeId === 4 / designation)
  if (filterTypeId == 4 && DNID) {
    queryParams.DNID = DNID;
    // Optional: name/code search একসাথে designation এর সাথে
  ...
  // ✅ UserAction filter — Always dynamic (independent of filterTypeId)
  if (userAction !== undefined && userAction !== "" && userAction !== null) {
    queryParams.UserAction = userAction;
  }

  // ✅ RTK Query
  const { data: usersData, isLoading, isError, refetch } = useGetFilteredTeachersQuery(queryParams);
  ...
  const brandData = usersData?.teachers || [];
  ...
  const handleTeacherCreateModel = useCallback(() => {
    showModal('শিক্ষাক তৈরি করুন', 'ADD_TEACHER_INFO');
  }, []);

  const handleTeacherUpdateModel = useCallback((id) => {
    showModal('শিক্ষাক আপডেট করুন', 'UPDATE_TEACHER_INFO', id);
  }, []);
  ...
  const filterTypeOptions = [
    ...
    { FilterTypeId: "4", FilterTypeName: "পদবী হিসাবে" },
  ];
      ...
      <FormProvider {...methods}>
        ...
        <div className="px-6 py-5 bg-white flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-[22px] font-bold text-gray-900 tracking-tight leading-none">শিক্ষক তালিকা</h2>
              <p className="text-[13px] text-gray-500 mt-1.5 font-medium">সিস্টেমের শিক্ষক পরিচালনা ও দেখুন</p>
            </div>
          </div>
          <button onClick={handleTeacherCreateModel} className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg text-sm font-medium flex items-center gap-2 transition-all shadow-sm shadow-blue-200">
        ...
        <div className="px-6 py-5 flex flex-col md:flex-row justify-between items-center bg-white gap-4 border-t border-gray-50">
          ...
          <div className="flex items-center gap-2 w-full md:w-[520px]">
            ...
            {/* DNID Select (only for designation filter) */}
            {filterTypeId == 4 && (
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-[#f8fafc] border-y border-gray-100 text-gray-600">
                  ...
                  <th className="py-3.5 px-4 font-bold text-[13px] text-center">পদবী</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {brandData.length > 0 ? (
                  brandData.map((brand, key) => {
                    ...
                    const isActive = user.UserAction === 1;
                    ...
                    return (
                      <tr key={key} className="hover:bg-gray-50/50 transition-colors">
                          ...
                          <div className="text-center">
                            <ViewPermission permissionId={permissionsDataList.user_entry} permissionType="edit">
                              <RoundedCropImageUpload
                                label=""
                                registerKey={`avatar-${user.UserID}`}
                                require
                                image={imageSrc}
                                fallbackImage={avaterImage}
                                previewUrl={preview[user.UserID] || null}
                                setPreviewUrl={(value) => {
                                  setPreview((prev) => ({ ...prev, [user.UserID]: value }));
                                }}
                                size="w-16 h-16"
                                userId={user.UserID}
                        ...
                        <td className="py-2 px-4">
                          <div className="font-semibold text-gray-800 text-[15px] leading-tight whitespace-nowrap">{user.UserName}</div>
                          <div className="text-gray-500 font-medium text-[13px] mt-1">{user.Mobile1 || '-'}</div>
                        </td>
                        ...
                        <td className="py-2 px-4">
                          <div className="flex items-center justify-center gap-2">
                            <button
                              onClick={() => handleTeacherUpdateModel(user.UserID)}
        ...
        <div className="px-6 py-2 flex flex-col sm:flex-row justify-between items-center bg-white gap-4">
          <div className="text-[13px] text-gray-500 font-normal">
            Showing <span className="font-semibold text-gray-700">{startRecord}</span> to <span className="font-semibold text-gray-700">{endRecord}</span> of <span className="font-semibold text-gray-700">{totalUsers}</span> teachers
          </div>
