// src/Admin/pages/AdminAllMadrasah.jsx
...
import { useUpdateRedirectMutation } from "../../features/Admin/redirectSlice"; // <-- নতুন API
import useTranslate from "../../utils/Translate";

const AdminAllMadrasah = ({ pageTitle }) => {
  ...
  useEffect(() => {
    setIsLoading(isFetchingMadrasah || isFetchingStats || isToggling || isUpdatingRedirect);
  }, [isFetchingMadrasah, isFetchingStats, isToggling, isUpdatingRedirect]);
  ...
  // 🌟 Handler for redirect on/off
  // const handleToggleRedirect = useCallback(async (row) => {
  //   // backend থেকে onTestStatus আসছে (0/1)
  //   const currentRedirectState = row.onTestStatus === 1;
  //   const newValue = currentRedirectState ? 0 : 1;
  //   const actionText = currentRedirectState ? 'disable' : 'enable';

  //   const result = await Swal.fire({
  //     title: currentRedirectState ? 'Turn Off Redirect?' : 'Turn On Redirect?',
  //     text: `Do you want to ${actionText} redirect for this madrasah?`,
  //     icon: 'question',
  //     showCancelButton: true,
  //     confirmButtonColor: '#3085d6',
  //     cancelButtonColor: '#d33',
  ...
  //       Swal.fire({
  //         title: 'Success!',
  //         text: `Redirect ${actionText}d successfully.`,
  //         icon: 'success',
  //         timer: 1500,
  //         showConfirmButton: false,
  //       });
  //     } catch (error) {
  //       Swal.fire('Error!', 'Failed to update redirect status.', 'error');
  //     }
  //   }
  // }, [updateRedirect]);
  ...
  // 🌟 Handler for redirect on/off
  const handleToggleRedirect = useCallback(async (row) => {
    // Number() ব্যবহার করা হলো যাতে string "1" আসলেও তা নাম্বারে কনভার্ট হয়
    const currentRedirectState = Number(row.onTestStatus) === 1;
    const newValue = currentRedirectState ? 0 : 1;
    const actionText = currentRedirectState ? 'disable' : 'enable';
    ...
    if (result.isConfirmed) {
      try {
        ...
        // 🌟 অপশনাল কিন্তু ভালো: যদি mutation এ invalidatesTags ঠিকমতো কাজ না করে, 
        // তবে এখানে আপনি লোকাল স্টেট (users) ম্যানুয়ালি আপডেট করতে পারেন।
        setUsers(prevUsers => {
          if (!prevUsers?.data) return prevUsers;
          const updatedData = prevUsers.data.map(u => 
            u.UserCode === row.UserCode ? { ...u, onTestStatus: newValue } : u
          );
          return { ...prevUsers, data: updatedData };
        });
  ...
  // 🌟 Open the DefaultModal with the new view 🌟
  ...
  const headerFormatter = (title, field, isSortable = true) => {
    ...
    return (
      <div
        className="font-default flex items-center justify-center gap-1 cursor-pointer hover:text-blue-600 transition-colors"
  ...
  const columnsMadrasah = [
    ...
    },
    {
      ...
      render: (row) => (
        <div className="flex items-center justify-center">
          <label className="relative inline-flex items-center cursor-pointer" title="Toggle Redirect">
            <input
              type="checkbox"
              // checked={row.onTestStatus === 1}
              checked={Number(row.onTestStatus) === 1}
              onChange={() => handleToggleRedirect(row)}
    ...
    },
    {
      ...
      render: (row) => (
        <div className="flex items-center justify-center space-x-2">
          {/* Status toggle outside modal */}
          <label className="relative inline-flex items-center cursor-pointer" title="Toggle Status">
            <input
              type="checkbox"
          ...
          {/* Three-dot button for other actions */}
          <button
...
export default AdminAllMadrasah;
