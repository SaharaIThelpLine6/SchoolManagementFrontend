...
const SitPlanAssign = ({ sessionId, examId, sharedStepData }) => {
    ...
    // assignments: { "hallId-colIdx-rowIdx-seatNum" -> { cls, userCode, shiftId } }
    ...
    const { data: sitPlanDetails } = useGetSitPlanBySitPlanIDQuery({ sitplanid: sharedStepData }, {skip: !sharedStepData});
    ...
    function doAssign() {
        ...
        if (shiftConflict) {
            ...
            Swal.fire({
                ...
                text: `এই ক্লাস ইতিমধ্যে "${lockedShiftName}" শিফটে নির্ধারিত।`,
        ...
        if (remainingStudents === 0) {
            ...
            Swal.fire({
                icon: "error",
                title: "সাবমিশন ব্যর্থ!",
                text: `All students of this class are already seated.`,
                confirmButtonColor: "#3085d6",
                confirmButtonText: "ঠিক আছে",
            });
            return;
        ...
        if (freeSlots.length === 0) {
            ...
            Swal.fire({
                icon: "error",
                title: "সাবমিশন ব্যর্থ!",
                text: `All selected seats are already allocated.`,
                confirmButtonColor: "#3085d6",
                confirmButtonText: "ঠিক আছে",
            });
            return;
        ...
        if (duplicateCount > 0) {
            ...
            Swal.fire({
                icon: "success",
                title: "সাবমিশন সফল!",
                text: `${freeSlots.length} seats assigned. ${duplicateCount} seats were already booked.`,
                confirmButtonColor: "#3085d6",
                confirmButtonText: "ঠিক আছে",
            });
        } else {
            Swal.fire({
                icon: "success",
                title: "সাবমিশন সফল!",
                text: `Assigned ${freeSlots.length} seats to ${getClassMeta(selectedClass)?.label}`,
                confirmButtonColor: "#3085d6",
                confirmButtonText: "ঠিক আছে",
            });
    ...
    // ── render ────────────────────────────────────────────────────────────────
    return (
        <div className={`p-5 ${currectLanguage == 'en' ? 'font-lato' : 'font-SolaimanLipi'}`}>
            <div className="mb-5">
                ...
                <p className="text-sm text-gray-400 mt-1">
                    {activeHall ? (
                        ...
                        'Select a hall below to begin assigning seats.'
                    )}
                </p>
            </div>

            <div className="grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-4 items-start relative">
                    ...
                    {hallList.map(hall => {
                        ...
                        return (
                            ...
                            >
                                {/* Accordion header */}
                                <div
                                    className={`flex items-center justify-between px-4 py-3 cursor-pointer select-none transition-colors ${isActive ? 'bg-blue-50' : 'bg-white hover:bg-gray-50'
                                        }`}
                                                        ...
                                                        {col.rows.map((row, ri) => (
                                                            ...
                                                            >
                                                                ...
                                                                {Array.from({ length: row.seats }).map(
                                                                    (_, si) => {
                                                                        const sn = si + 1;
                                                                        const key = `${selectedShift}-${hall.id}-${ci}-${ri}-${sn}`;
                                                                        ...
                                                                        return (
                                                                            <button
                                                                                ...
                                                                                title={ assignedVal? `${c?.label} · ${assignedVal.userCode}`: `${col.label} · Row ${ri + 1} · Seat ${sn}`}
                                                                                onClick={() => clickSeat(hall.id, ci, ri, sn)}
                                                                                style={c ? { background: c.hex, borderColor: c.hexBorder, color: c.hexText,} : {}}
                                                                                className={`w-8 h-8 rounded-md border text-[9px] flex items-center justify-center transition-all shrink-0 font-medium overflow-hidden ${c ? 'cursor-pointer' : isStart ? 'bg-blue-100 border-blue-400 text-blue-700 cursor-pointer' : isPreview ? 'bg-blue-50 border-blue-200 text-blue-400 opacity-70 cursor-pointer' : 'bg-white border-gray-200 text-gray-400 hover:bg-gray-50 hover:border-gray-300 cursor-pointer' }`}>
                                                                                {assignedVal ? assignedVal.userCode : isStart ? '▶' : sn}
                                                                            </button>
                                                                        );
                                                                    }
                                                                )}
                ...
                {/* ── Right panel ───────────────────────────────────────── */}
                <fieldset disabled={isLocked} className={`flex flex-col gap-3 ${isLocked ? 'opacity-50 pointer-events-none select-none' : ''}`}>
                    <div className="sticky top-4 self-start">
                        ...
                        <div className="bg-white border border-gray-200 rounded-xl p-4">
                            ...
                            {/* scope */}
                            <div className="flex flex-col gap-1.5 mb-4">
                                <label className="text-xs text-gray-500">{ translate("Student Sit allocation") }</label>
                                <div className="flex flex-col gap-2">
                                    {SCOPE_OPTIONS.map(opt => (
                                        ...
                                        >
                                            ...
                                            {/* <p
                                                className={`text-xs ml-5 ${scope === opt.value ? 'text-blue-500' : 'text-gray-400'
                                                    }`}
                                            >
                                                {translate(opt.sub)}
                                            </p> */}
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <p className="text-xs text-gray-400 mb-0.5">
                                                Scope: {scopeOpt?.label} · Shift: {shiftName}
                                            </p>
