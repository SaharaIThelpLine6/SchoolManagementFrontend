...
const MonthDetermineFeeTable = () => {
  ...
  const { filteredSelectedPerStudentFee } = useSelector((state) => state.student);
  ...
  useEffect(() => {
    console.log("useGetFeeLandByAdmissionQuery response:", { data, error, isLoading });
  ...
  useEffect(() => {
    if (monthFeeList.length > 0) {
      ...
      console.log("Form state initialized with monthFeeList:", methods.getValues());
    }
  ...
  if (error) {
    ...
    return <p>{translate("Error loading data:")} {error.message || "Unknown error"}</p>;
  }
        ...
        <table className="min-w-full table-auto text-sm md:text-base">
          <thead className="bg-[#e9ebee] text-black">
            <tr>
              ...
              <th className="px-4 py-3 text-center">{translate("Comment")}</th>
