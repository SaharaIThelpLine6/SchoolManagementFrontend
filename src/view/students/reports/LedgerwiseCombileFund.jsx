...
const  LedgerwiseCombileFund = ({ reportData, query }) => {
      ...
      <div className="bg-white pt-8">
        <table className="w-full border-collapse border border-black bg-white">
          <thead>
            <tr className="bg-white text-sm text-black">
              <th className="border border-black  bg-white p-2 text-[20px] text-start" colSpan={3}>{reportData.length > 0 && reportData[0]["GLMainName"] ? reportData[0]["GLMainName"] : reportData.length > 0 ? reportData[0]["GlName"] : null}</th>
            </tr>
          ...
          <tbody>
            {reportData.map((data, index) => (
              <tr>
                ...
                <td className="bg-[#fff] border border-black text-center p-2 text-[18px]">{data.SlName}</td>
                <td className="bg-[#fff] border border-black text-center p-2 text-[18px]">{data.Amount}</td>
              </tr>
            ))}
        ...
        <div className="text-end pt-[20px]">
          <p className="text-[24px] font-semibold leading-[28px]">
            মোট:{" "}
            {reportData.reduce((total, item) => total + Number(item.Amount || 0), 0)}
          </p>
...
export default SubLedgeWisePrint;