...
const DepositCostSubLedgerReport = ({ reportData, query }) => {
            ...
            <div className="bg-white pt-8">
                ...
                <table className="w-full border-collapse border border-black bg-white mb-8">
                    <thead>
                        ...
                        <tr>
                            <th className="text-[20px] text-start py-2 bg-white ps-2" colSpan={6}>সাব লেজার: {reportData[0]["SlName"]}</th>
                        </tr>
                    </thead>
                    <tbody>
                        {
                            reportData.map((data, index) => (
                                <tr>
                                    <td className="text-[16px] py-2 px-1 border border-black text-center bg-white">{index + 1}</td>
                                    <td className="text-[16px] py-2 px-1 border border-black text-center bg-white">{data.TransactionDateEng}</td>
                                    <td className="text-[16px] py-2 px-1 border border-black text-center bg-white">{data.VoucherNo}</td>
                                    <td className="text-[16px] py-2 px-1 border border-black text-center bg-white">{data.BookNo}</td>
                                    <td className="text-[16px] py-2 px-1 border border-black text-center bg-white">{data.Particulars}</td>
                                    <td className="text-[16px] py-2 px-1 border border-black text-center bg-white">{data.Amount}</td>
                                </tr>
                            ))
                        }
...
export default DepositCostSubLedgerReport;