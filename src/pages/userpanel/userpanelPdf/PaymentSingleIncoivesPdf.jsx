...
const PaymentSingleIncoivesPdf = ({ invoices }) => {
  ...
  return (
    <div className="container mx-auto p-4 space-y-8">
      {invoices.map((invoice) => (
        <div
          key={invoice.GOPIID}
          className="w-full bg-white border rounded shadow p-4 md:p-6"
        >
          {/* Header */}
          <div className="flex flex-col md:flex-row justify-between items-center mb-4">
            <h2 className="text-xl font-bold mb-2 md:mb-0">ইনভয়েস</h2>
            <span className="text-sm text-gray-500">#{invoice.TransactionID}</span>
          </div>
                ...
                {invoice.InvoiceDetails && invoice.InvoiceDetails.length > 0 ? (
                  invoice.InvoiceDetails.map((item) => (
                    <tr key={item.GOPIDID} className="text-sm text-gray-700">
                      <td className="border border-gray-300 px-3 py-2">{item.FeeType}</td>
                      <td className="border border-gray-300 px-3 py-2">{item.MonthName}</td>
                      <td className="border border-gray-300 px-3 py-2">{item.SessionName}</td>
