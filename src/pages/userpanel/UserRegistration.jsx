...
import { usePostUserPhoneMutation, usePostUserRegisterMutation, usePostVerifyTokenMutation } from "../../features/userPanel/userRegistration/userRegistrationQuerySlice";
...
export function StepOne({ phone, otpTimer, onResend, studentCode }) {
    return (
        <>
            {/* Step indicator ... */}
            <ol className="flex items-center w-full space-x-4 mb-4 max-w-[200px] mx-auto"><li className="flex w-full items-center after:content-[''] after:w-full after:h-1 after:rounded-full after:border-b after:border-info after:border-4 after:inline-block after:ms-4 after:rounded-full"> <span className="flex items-center justify-center w-10 h-10 bg-neutral-tertiary rounded-full lg:h-12 lg:w-12 shrink-0 text-white bg-info border border-info"> <svg className="w-[26px] h-[26px]" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24"><path stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 9h3m-3 3h3m-3 3h3m-6 1c-.306-.613-.933-1-1.618-1H7.618c-.685 0-1.312.387-1.618 1M4 5h16a1 1 0 0 1 1 1v12a1 1 0 0 1-1 1H4a1 1 0 0 1-1-1V6a1 1 0 0 1 1-1Zm7 5a2 2 0 1 1-4 0 2 2 0 0 1 4 0Z" /></svg> </span> </li> <li className="flex items-center"> <span className="flex items-center justify-center w-10 h-10 bg-neutral-tertiary rounded-full lg:h-12 lg:w-12 shrink-0 text-info border border-info"> <svg className="w-5 h-5 text-body" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24"><path stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 4h3a1 1 0 0 1 1 1v15a1 1 0 0 1-1 1H6a1 1 0 0 1-1-1V5a1 1 0 0 1 1-1h3m0 3h6m-6 7 2 2 4-4m-5-9v4h4V3h-4Z" /></svg> </span> </li> </ol>


            {!phone && (<DefaultInput registerKey={"StudentCode"} label={"Student ID"} placeholder={"Enter Student ID"} type="text" />)}


            {phone && (
                <div className="">
                    <p className="text-[#d6d6d6] text-[18px] bg-white block border border-[#d6d6d6] rounded-[5px] py-1 pl-2">{studentCode}</p>
                    <p className="text-[#d6d6d6] text-[18px] bg-white block border border-[#d6d6d6] rounded-[5px] py-1 mt-2 pl-2">{phone}</p>

                    <p className="text-blue-700 text-sm mt-1">
                        OTP sent to your phone. Time left: <strong>{otpTimer}s</strong>
                    </p>

                    <DefaultInput registerKey={"otp"} placeholder={"Enter OTP"} type="number" require={true} />
                    {otpTimer === 0 && (
                        <button
                            onClick={onResend}
                            type="button"
            ...
            <ol className="flex items-center w-full space-x-4 mb-4">
                <li className="flex w-full items-center text-fg-brand after:content-[''] after:w-full after:h-1 after:border-b after:border-info after:border-4 after:inline-block after:ms-4 after:rounded-full">
                    <span className="flex items-center justify-center w-10 h-10 border border-info rounded-full lg:h-12 lg:w-12 shrink-0 text-info">
                        <svg className="w-[36px] h-[36px]" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="50" height="50" fill="none" viewBox="0 0 24 24"><path stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 11.917 9.724 16.5 19 7.5" /></svg>
                    </span>
                </li>
                <li className="flex w-full items-center after:content-[''] after:w-full after:h-1  after:rounded-full after:border-b after:border-info after:border-4 after:inline-block after:ms-4 after:rounded-full">
                    <span className="flex items-center justify-center w-10 h-10  bg-neutral-tertiary rounded-full lg:h-12 lg:w-12 shrink-0 text-info border border-info">
                        <svg className="w-[26px] h-[26px]" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24"><path stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 9h3m-3 3h3m-3 3h3m-6 1c-.306-.613-.933-1-1.618-1H7.618c-.685 0-1.312.387-1.618 1M4 5h16a1 1 0 0 1 1 1v12a1 1 0 0 1-1 1H4a1 1 0 0 1-1-1V6a1 1 0 0 1 1-1Zm7 5a2 2 0 1 1-4 0 2 2 0 0 1 4 0Z" /></svg>
                    </span>
                </li>
            ...
            <div className="w-full h-full sm:h-auto md:max-w-md bg-[#ddeffe] flex flex-col">
                ...
                <FormProvider {...methods}>
                    <form onSubmit={handleSubmit(onSubmit)} className="max-w-sm mx-auto mt-6 w-full px-4">
                        ...
                        {steps.map((s, index) => (
                            <div key={index} style={{ display: index === currentStepIndex ? 'block' : 'none' }}>
                                {index === 0
                                    ? <StepOne
                                        phone={phoneNumber}
                                        otpTimer={otpTimer}
