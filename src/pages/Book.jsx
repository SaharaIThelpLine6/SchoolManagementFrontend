...
import { useDeleteAcademicSubjectMutation, useGetAcademicSubjectsQuery } from "../features/class/classQuerySlice";
...
const Book = ({ pageTitle }) => {
  ...
  const { data: academicSubjectData = [], isError, isLoading } = useGetAcademicSubjectsQuery();
  const [deleteSubject, { isLoading: isDeleting }] = useDeleteAcademicSubjectMutation();
  ...
  const handleEditSubject = useCallback((id) => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        Swal.fire({
          ...
          text: error.data?.message || translate("Failed to delete subject"),
          icon: "error",
        });
      }
    }
