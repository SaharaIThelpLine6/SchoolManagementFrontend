...
import { useGetStudentBySearchQuery, useGetStudentReportCetsQuery, useGetStudentReportsMutation, useGetStudentReportTypeQuery, usePostStudentCharacterReportMutation } from '../features/student/studentQuerySlice';
        ...
        <div>
            <div className="py-8 px-6 hidden_in_print">
                <FormProvider {...methods}>
                    <form onSubmit={methods.handleSubmit(onSubmit)} className="mx-auto flex justify-between items-center w-full">
                        <div className='relative max-w-[400px] w-full'>
                            <div className="w-full">
                                <label htmlFor={"filterStudentCode"} className="mb-1 block text-black font-SolaimanLipi">
                                    <span className="text-red-500">
                                        {translate("User Code")} * :
                                    </span>
                                </label>
                                <input type="text" {...methods.register("filterStudentCode", { required: " " })} className='w-full rounded border-[1.5px] border-stroke bg-[#EDEDED] px-2 h-[38px] text-black outline-none text-[14px] transition focus:border-primary active:border-primary disabled:cursor-not-allowed disabled:bg-slate-200' onInput={() => setUserTyping(true)} autoComplete='false' />
                            </div>
