...
import { useDeleteExamNameMutation, usePostNewExamMutation, useUpdateExamnameMutation } from "../features/exam/examQuerySlice";
...
const Exam = ({ pageTitle }) => {
    ...
    const { data: examNames, error: examNameError, isLoading } = useGetExamNamesQuery();
    const dispatch = useDispatch();
    const translate = useTranslate();
    const [addExam, { isLoading: examNameInsertLoding, isError, isSuccess }] = usePostNewExamMutation();
    const [updateExam, { isLoading: examNameUpdateLoading, isError: examNameUpdateError, isSuccess: examnameUpdateSuccessful }] = useUpdateExamnameMutation();
    const [deleteExam, { isLoading: examNameDeleteLoading, isError: examNameDeleteError, isSuccess: examnameDeleteSuccessful }] = useDeleteExamNameMutation();
    useEffect(() => {
    ...
    const onSubmit = async (data) => {
        ...
        try {
            if (editMode === 0) {
                ...
                console.log("Edit mode is active, but no update logic implemented yet.");
                const body = {
                    ID: editMode,
                    ...data
                }
