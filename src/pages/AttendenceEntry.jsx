...
import { useGetTimeSettingsFilteredAttendanceQuery, useGetTimeShiftingsQuery, useGetTimeSwitchesQuery } from "../features/attendance/attendanceSlice";
...
const AttendenceEntry = ({ pageTitle }) => {
  ...
  const { watch } = method;
  const [ShiftID, UserTypeID, SessionID, ClassID, ResidentialID, UserCode, SwitchID] = watch([
    "ShiftID",
    "UserTypeID",
    "SessionID",
    "ClassID",
  ...
  const { data, isLoading, error } = useGetTimeSettingsFilteredAttendanceQuery({
    UserTypeID,
    SessionID,
    ClassID,
    ShiftID,
  ...
  // সরাসরি data?.data থেকে filter করা হচ্ছে, কোনো আলাদা state এ রাখা হচ্ছে না
      ...
      <div className="font-lato bg-white p-4 sm:p-6 rounded-xl shadow-lg">
        ...
        <div className="bg-gray-50 p-4 rounded-lg mb-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-6 gap-4">
            ...
            />
            <DefaultSelect
              label={translate("শিডিউল নাম")}
              options={switchLists?.data ?? []}
              valueField="ID"
              nameField="TypeNameBangla"
              registerKey="SwitchID"
            />
            {/* <DefaultInput
            ...
            <div className="overflow-x-auto">
              <div className="min-w-[900px]">
                <div className="grid grid-cols-[56px_90px_1fr_1fr_130px_100px_100px_100px] bg-blue-50 px-2 py-2 text-[12px] font-bold text-blue-700 border-b border-blue-100">
                  ...
                  <span>{translate("টাইম")}</span>
                  <span>{translate("হাজিরা")}</span>
                </div>

                <div className="max-h-[480px] overflow-y-auto">
                  {isLoading ? (
                    <div className="flex flex-col items-center justify-center gap-2 py-16 text-slate-400">
                      <p className="text-[13px]">{translate("লোড হচ্ছে...")}</p>
                    </div>
                  ) : error ? (
                    <div className="flex flex-col items-center justify-center gap-2 py-16 text-red-400">
                      <p className="text-[13px]">{translate("ডাটা আনতে সমস্যা হয়েছে")}</p>
                    </div>
                  ) : rows.length === 0 ? (
                    <div className="flex flex-col items-center justify-center gap-2 py-16 text-slate-400">
                      <SvgIcon name="search" className="h-8 w-8" />
                      <p className="text-[13px]">{translate("কোনো শিক্ষার্থী নেই")}</p>
                    </div>
                  ) : (
                    rows
                      .filter((s) => {
                        ...
                        >
                          ...
                          <span className="text-slate-700 truncate pr-2 text-[12px]">
                            <TimePicker
                              ...
                              registerKey={`VacationTimeTo_${s.UserID}`}
                              require={true}
                            />
                          </span>
                          <span className="text-slate-700 truncate pr-2 text-[12px]">
                            <ToggleSwitch
                              checked={activeUser === s.UserID}
                              onChange={(e) =>
                                handleToggle(s.UserID, e.target.checked)
                              }
                            />
                          </span>
