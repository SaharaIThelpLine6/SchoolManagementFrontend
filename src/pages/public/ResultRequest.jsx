...
const ResultRequest = () => {
  ...
  useEffect(() => {
    if (resultStatus === "failed" && !toastShown.current) {
      ...
      toast.dark("দুঃখিত, কোন তথ্য পাওয়া যায়নি!", {
        className:
          "toast-error-container toast-error-container-after min-h-[50px] max-h-[50px] overflow-hidden text-[14px] font-SolaimanLipi bg-[#323232] text-[#ffffff] py-2 px-2 rounded-[4px] font-normal",
        style: {
          boxShadow:
              ...
              <div className="relative z-20 bg-transparent">
                ...
                >
                  ...
                  {classList.map((session) => (
                    <option
                      key={session.SubClassID}
                      value={session.SubClassID}
                      className="text-body"
                    >
