...
const PointBasedResultCreateUpdate = ({ pageTitle }) => {
  ...
  /**
   * Keyboard navigation handler for table inputs.
   *
   * - Enter  → move DOWN to the same column (same subject) in the next row
   * - Tab    → move RIGHT to the next subject column in the same row
   *            (Shift+Tab → move LEFT to the previous subject column)
   *
   * Each <TableInput> must receive:
   *   data-row={rowIndex}   – 0-based index within the current page
   *   data-col={colIndex}   – 0-based index of the visible subject column
   */
