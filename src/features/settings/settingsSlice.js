...
export const fetchSettingsData = createAsyncThunk(
  ...
  async (_, { getState }) => {
    ...
    const [
      ...
      userTypeResponse,
      studentFinancialStatusResponse,
    ] = await Promise.all([
...
export const fetchDidata = createAsyncThunk(
  'settings/fetchDidata',
  async (id, { getState }) => {
    ...
    const response = await getSettingsData(
      token,
      `/api/settings/district?divition_id=${id}`
    );
...
export const fetchThanadata = createAsyncThunk(
  ...
  async (id, { getState }) => {
    ...
    const response = await getSettingsData(
      token,
      `/api/settings/thana?district_id=${id}`
    );
    return { id, data: response };
  }
);
